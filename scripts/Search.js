...
class Search {
  ...
  async getSearchData() {
    try {
      ...
      let datastoreResources = packageData["result"]["resources"].filter(
      ...
      const data = await neighbourhoods.getDatastoreResource(
        datastoreResources[0]
      );
  ...
  searchFunction() {
    ...
    this.searchBar.addEventListener("input", () => {
      ...
      if (query.length >= 2) {
        ...
        const filteredNeighbourhoods = this.allNeighbourhoods.filter(
          (neighbourhood) => neighbourhood.name.toLowerCase().includes(query)
        );
  ...
  highlightGeometry(neighbourhood) {
    ...
    try {
      ...
      console.error(
        "Failed to parse geometry for neighbourhood:",
        neighbourhood,
        error
      );
    }
    ...
    geoJsonLayer
      .bindPopup(
        `<strong>${neighbourhood.name}</strong><br>Population: ${neighbourhood.population}`
      )
      .openPopup();
